...
getopts('f:m:n:', \%opts); # getting options from the command line. f is the fasta file, m is the max length, n is the min length
...
while (my $header = <FA>) { # Start the loop on the fast file
	...
	if (length $seq > $opts{m} or length $seq < $opts{n}) { # Check if the length of the sequence is greater than the maximum allowed
